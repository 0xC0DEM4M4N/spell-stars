...
import { CalendarDays, ChevronLeft, ChevronRight, Printer, Star, Target } from "lucide-react";
import { Button } from "@/components/ui/button";

const WordChip = ({ word, index, week }) => (
  <div className="group flex items-center justify-between border border-white/10 bg-white/[0.04] px-4 py-3 transition-colors duration-300 hover:border-cyan-300/40 hover:bg-cyan-300/10" data-testid={`word-chip-week-${week}-${index + 1}`}>
...
const WeekCard = ({ week, active, current, onPrint, isDragging, onClick }) => (
  ...
  >
    ...
    <div className="mt-7 border-l-2 border-cyan-300 pl-5">
    ...
    <div className="mt-8 grid grid-cols-1 gap-3 sm:grid-cols-2" data-testid={`word-list-week-${week.week}`}>
      {week.words.map((word, index) => <WordChip key={word.text} word={word} index={index} week={week.week} />)}
    </div>
  </motion.article>
);
